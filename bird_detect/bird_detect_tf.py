...
class NN(object):
    ...
    @define_scope(initializer=tf.slim.xavier_initializer())
    def prediction(self):
        ...
        return tf.slim.fully_connected(net, int(self.num_out), tf.nn.softmax)
...
def main():
    ...
    for c in cnts:  # loop through each contour
        if cv2.contourArea(c) > 1200:
            ...
            label = misc.raw_input('0: no bird, 1: bird, 2: see whole map')
            try:
                label = int(label)
            except ValueError:
                misc.imshow(im_crop)
                label = int(misc.raw_input('Please enter only an integer'))
            im_copy = im.copy()
            if label == 2:
                while (1):
                    ...
                    if cv2.waitKey(10000) == 27:
                        ...
                        break
                label = int(misc.raw_input('0: no bird, 1: bird'))
