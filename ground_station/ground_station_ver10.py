...
ax7.set_ylim(127.202, 127.207)
ax7.set_xlim(34.6, 34.61)
...
def save_csv(data):
    time_passed, Ax, Ay, Az, Gx, Gy, Gz, lon, lat, detected = data
    with open('Cansat_data.csv', 'w') as f:
        f.write("Time, Ax, Ay, Az, Gx, Gy, Gz, Lon, Lat, Detected")
        f.write(
            time_passed + "," + Ax + "," + Ay + "," + Az + "," + Gx + "," + Gy + "," + Gz + "," + lon + "," + lat + "," + detected)
...
label = Tk.Label(root, image=logo, text='Ground Station for PBD', compound='left', fg='black', bg='white',
                 font=('times', 28), width=1400)
label.place(x=0, y=0)
